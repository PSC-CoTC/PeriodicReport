...
#could declare other controls in params?
#runs <- NA

#need LUs for stock and fishery units
#can at least partially engineer from dd_cohopsc_server


fram <- list(
  escp = framr::read_coho_escp(params$mdb) |> 
    filter(!is.na(RunYear))
  ,
  mort = read_coho_mort(params$mdb) |>
    filter(!is.na(RunYear)) |> 
...
**we need data for these** morts or catches? And, should we place these below ERs?  The treaty specifies ERs, not catches......
where are we placing ER estimates?
...
*for historical reference - I think these should be referenced and links provided, but not imbedded in the text and not under their own heading.  What do others think?*
<!-- link reports here -->
