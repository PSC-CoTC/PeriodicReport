...
fram$escp <- framr::read_coho_escp(params$mdb, runs = focal_run_ids, stocks = static$lu_stock$StockID)

fram$mort <- framr::read_coho_mort(params$mdb, runs = focal_run_ids, , stocks = static$lu_stock$StockID)
...
# #AK are 193:max?
# fram$mort_yr |> distinct(FisheryID, FisheryName) |> arrange(FisheryID) |>  print(n=300)

psc <- left_join(
  # join PSC metadata to mortality and re-aggregate,
  # then add columns of total mortality per year per re-aggregated PSC stock 
  # these are summed over fisheries and repeating per individual fishery
  fram$mort_yr |> 
    left_join(static$lu_stock |> select(StockID, PSC_StockID = PSCStock, PSC_StockName = Name), by = "StockID") |> 
    ...
    ungroup()
  ,
  #now join escapement re-aggregated to PSC stock per year, also repeating by fishery
  fram$escp |> 
  ...
  #now calc per-year-PSC stock abundance
  #and total and per-fishery ERs
  mutate(
    abund = mort_sum + escp,
...
```{r}
psc_gg <- list()
#looks like need to do some digging to sort facet overlap if going ggplotly route: https://github.com/plotly/plotly.R/issues/1224
```
...
```{r gt_ann_abund, fig.height=8, fig.width=12}
...
psc |> 
  distinct(RunYear, PSC_StockName, abund) |> 
  pivot_wider(names_from = RunYear, values_from = abund) |> 
  gt(caption = "Annual FRAM-estimated abundance (as escapement + harvest)") |> 
  fmt_integer(columns = `2004`:`2019`)
...
```{r}
plot_abund_bar("Georgia Strait ML")
```

```{r}
plot_abund_bar("Georgia Strait VI")
```
