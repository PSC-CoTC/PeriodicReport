...
psc <- left_join(
  fram$mort |> 
    ...
    left_join(
      static$lu_stock |> 
        select(StockID, PSC_StockID = PSCStock, PSC_StockName = Name),
      by = "StockID") |> 
  ...
  left_join(distinct(static$lu_stock, PSC_StockID = PSCStock, PSC_Region = Region), by = "PSC_StockID")
...
psc |> 
  distinct(RunYear, PSC_Region, PSC_StockName, abund) |> 
  pivot_wider(names_from = RunYear, values_from = abund) |> 
  gt(
    groupname_col = "PSC_Region",
    caption = "Annual FRAM-estimated abundance (as escapement + harvest)"
    ) |> 
  cols_hide(`1998`:`2010`) |> 
  fmt_integer(columns = -PSC_StockName)

```

## Exploitation Rates

```{r gt_ER, fig.height=8, fig.width=12}
psc |> 
