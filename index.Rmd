...
# #probably wrong but demoing...
# fram$escp |> 
#   left_join(lu_stock, by = "StockID") |> 
#   ggplot(aes(RunYear, escp, fill = StockLongName)) + 
#   geom_col() + facet_wrap(~Name, scales = "free")

#among other things, need to confirm order of operations of lu_stock join and aggregate
left_join(
  fram$escp
  ,
  fram$mort |> 
    group_by(RunYear, StockID, StockLongName) |> 
    summarise(across(mort, sum), .groups = "drop")
  ,
  by = c("RunYear", "StockID", "StockLongName")
  ) |> 
  left_join(lu_stock, by = "StockID") |> 
  group_by(RunYear, PSCStock, Name) |> 
  summarise(across(c(escp, mort), sum), .groups = "drop") |> 
  mutate(
    abun = escp + mort, #this matches the "post-season" cohort value in DD tables 6.1:6.3
    er_tot = mort / abun
  ) |> 
  ggplot(aes(as.numeric(RunYear), er_tot)) +
  geom_col() + geom_smooth() +
  facet_wrap(~PSCStock+Name, scales = "free", labeller = label_wrap_gen(multi_line = F))
