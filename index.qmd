...
static <- list(
  ...
  releases = readxl::read_excel(
    ...
    col_types = c(rep('text',6), 'numeric', 'text', rep('numeric',6))) |> 
    mutate(across(where(is.character), ~replace_na(.,"")))
)
...
```{r fig_wild_stock_tagging}
#pending some changes to the MU strings from CCT
static$releases |> 
  filter(
    rearing_type != "H",
    MU %in% c(
  ...
  mutate(MU = if_else(MU %in% c("GSMLND","GSVNCI"), "GeorgSt", MU)) |> 
  select(MU, brood_year, CWT_Clipped, CWT_Unclipped) |> 
...
static$releases |> 
  ...
  mutate(MU = if_else(MU %in% c("GSMLND","GSVNCI"), "GeorgSt", MU)) |> 
  filter(
    !str_detect(hatchery_location_name, "chools|CHOOL| ES| MS| HS| FFA")
  ) |>
  select(MU, brood_year, tag_code_or_release_id, CWT_Clipped, CWT_Unclipped, CWT_Unclipped_DIT) |> 
  pivot_longer(cols = -c(MU, brood_year, tag_code_or_release_id), names_to = "tag_mark", values_to = "val") |> 
  drop_na(val) |> 
...
```{r fig_hatch_stock_releases_mu_cwt_only}
#pending some changes to the MU strings from CCT
static$releases |> 
  filter(
    rearing_type == "H",
  ...
  filter(
    !str_detect(hatchery_location_name, "chools|CHOOL| ES| MS| HS| FFA")
  ) |> #pull(hatchery_location_name) |> unique()
  select(MU, brood_year, tag_code_or_release_id, CWT_Clipped, CWT_Unclipped, CWT_Unclipped_DIT) |> 
  pivot_longer(cols = -c(MU, brood_year, tag_code_or_release_id), names_to = "tag_mark", values_to = "val") |> 
  drop_na(val) |> 
  mutate(tag_mark = factor(tag_mark, levels = names(pal_rel_tag_mark))) |> 
  group_by(MU, brood_year, tag_mark) |> summarise(val = sum(val), .groups = "drop") |> 
  filter(between(brood_year,1980,2020)) |> 
  ggplot() +
  geom_col(aes(brood_year, val, fill = tag_mark)) +
  scale_fill_manual(values = pal_rel_tag_mark) +
...
static$releases |> 
  ...
  select(MU, brood_year, tag_code_or_release_id, CWT_Clipped, CWT_Unclipped, CWT_Unclipped_DIT, MM, Unclipped) |> 
  pivot_longer(cols = -c(MU, brood_year, tag_code_or_release_id), names_to = "tag_mark", values_to = "val") |> 
  drop_na(val) |> 
  mutate(tag_mark = factor(tag_mark, levels = names(pal_rel_tag_mark))) |> 
  group_by(MU, brood_year, tag_mark) |> summarise(val = sum(val), .groups = "drop") |> 
