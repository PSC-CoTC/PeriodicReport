...
In years prior to 2014, Canadian fisheries were managed so as not to exceed a 3% maximum {{< glosarry ER >}}. In May 2014, Canada decided to permit up to a 16% {{< glosarry ER >}} on upper Fraser Coho Salmon in Canadian fisheries to allow for impacts in fisheries directed at a record Fraser Sockeye forecast. Since 2015, upper Fraser Coho Salmon in Canadian fisheries have been managed per low status limitations. The projected status of Canadian coho {{< glosarry MUs >}} in 2024 indicates continuing concerns for the condition of Interior Fraser Coho Salmon. The Interior Fraser Coho Salmon {{< glosarry MU >}} is anticipated to remain in low abundance status, resulting in a requirement to constrain the total mortality fishery {{< glosarry ER >}} for 2024 Southern U.S. fisheries to a maximum of 10.0%.
...
#| fig-caption: Total number of wild Coho Salmon smolts caught, tagged, and released by clip status for each Management Unit over brood years 1983 through 2020 (data downloaded from RMIS 9-6-2023).
...
ToDo: create this image from the excel file: (2/5/25 I attempted code, but know it won't likely work!)
![](images/Skagit_marine_SR.png){width=100% fig-alt="Skagit Marine Survival Rates"}

```{r fig_msr_skagit, fig.height=4, fig.width=6}
#| fig-cap: Estimated marine survival rates for the Skagit MU Baker River Wild Coho Salmon, (data provided by M. Litz, WDFW).
#| label: fig-msr-skagit
plot_msr("Skagit")
```
...
```{r fig_hatch_stock_releases_skagit, fig.height=6.5, fig.width=6.5}
...
**Skagit {{< glossary MU >}} average annual (total) and time period specific {{< glossary ERs >}} used in the current FRAM base period.** (Question for Collin --> is there a way to autobuild these tables directly from the FRAM database?)
...
plot_ns_msf("Skagit")
```
ToDo: add a figure to depict the following:
...
note - not sure we need this table anymore.  seems redundant 
